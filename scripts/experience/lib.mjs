...
function hashFile(file) {
  // Materiality is a source-level signal, so a Git checkout's line-ending
  // policy must not make an unchanged experience look different in CI.
  return hashText(readFileSync(file, "utf8").replace(/\r\n?/g, "\n"));
}
...
function defaultRoles(route) {
  ...
  if (/share|embed|\/p\/|\/u\/|update|wedding-planning/.test(route)) {
    return ["owner", "guest", "viewer"];
  }
  return ["public"];
}

function defaultReviewTier(route, archetype) {
  if (route === "/" || route === "/app" || /sign-in|sign-up|invite|redeem|share|privacy|terms|security/.test(route)) {
    return "critical";
  }
...
function resolveSource(repoRoot, source) {
  const local = source.startsWith("notes/") ? source.slice("notes/".length) : source;
  return path.join(repoRoot, local.replaceAll("/", path.sep));
}
...
function discoverRoutes({ repoRoot, config, registrations }) {
  ...
  for (const file of walk(appRoot)) {
    ...
    const id = `${config.product.id}.${kind}.${suffix}`;
    entries.push(
      baseEntry({
        ...
        sourceFile: file,
        overrides: { id, ...(registrations.routeOverrides?.[id] ?? {}) },
      }),
    );
  }
  return entries;
...
function explicitEntries({ repoRoot, registrations }) {
  return registrations.surfaces.map((surface) => {
    const sourceFile = resolveSource(repoRoot, surface.source);
    return baseEntry({
...
export function discoverRegistry({ repoRoot, config, registrations }) {
  const experiences = [
    ...discoverRoutes({ repoRoot, config, registrations }),
    ...explicitEntries({ repoRoot, registrations }),
...
export function validateRegistry({ registry, discovered, repoRoot }) {
  ...
  for (const breakpoint of REQUIRED_BREAKPOINTS) {
    const viewport = registry.breakpoints?.[breakpoint];
  ...
  for (const entry of registry.experiences) {
    ...
    for (const exception of entry.intentionalExceptions ?? []) {
      for (const field of ["id", "rationale", "owner", "scope", "approvalSource", "expiresAt", "remediationPlan"]) {
        if (!isNonEmptyString(exception[field])) errors.push(`${entry.id}: exception is missing ${field}`);
      }
      if (!exception.expiresAt || exception.expiresAt < today) {
        errors.push(`${entry.id}: expired exception ${exception.id ?? "<missing-id>"}`);
      }
    }
  }
  ...
  for (const [id, entry] of registeredById) {
    ...
    if (!existsSync(resolveSource(repoRoot, entry.source))) {
      errors.push(`${id}: broken source reference ${entry.source}`);
    }
  }
  return errors;
}
...
export function registryMetrics(registry) {
  ...
  const countPartial = (field) => experiences.filter((entry) => entry[field] === "partial").length;
  return {
    ...
    archetypes: Object.fromEntries(
      [...new Set(experiences.map((entry) => entry.archetype))]
        .sort()
        .map((archetype) => [archetype, experiences.filter((entry) => entry.archetype === archetype).length]),
    ),
    fixtureCoverageComplete: countComplete("fixtureCoverage"),
    fixtureCoveragePartial: countPartial("fixtureCoverage"),
    screenshotCoverageComplete: countComplete("screenshotCoverage"),
    screenshotCoveragePartial: countPartial("screenshotCoverage"),
    accessibilityCoverageComplete: countComplete("accessibilityCoverage"),
    accessibilityCoveragePartial: countPartial("accessibilityCoverage"),
  };
}
