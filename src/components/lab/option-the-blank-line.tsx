  ...
  return (
    <section className="tbl" aria-label="Notes hero — The Blank Line">
      ...
      <div className="tbl-wrap">
        ...
        {/* The settled artifact. role=img + full aria-label. */}
        <div
          className="tbl-artifact"
          role="img"
          aria-label="Signal Notes. A blank capture line with a live caret, ready to write. Below it, two notes with timestamps: the tasting note about the second red, 10:14; call back the celebrant before Thursday, 9:37. Three seconds from thought to written."
        >
...
.tbl-ov{
  ...
  letter-spacing:-0.035em;color:var(--soft);opacity:0;
...
.tbl-capture-hint{font-size:15px;letter-spacing:-0.005em;color:var(--faint);}
...
.tbl-item{
  ...
  padding:15px 2px;border-bottom:1px solid var(--hair-soft);
}
.tbl-item:first-child{border-top:1px solid var(--hair-soft);}
.tbl-item-text{
  flex:1;font-size:clamp(15px,.5rem+.6vw,17px);font-weight:500;
  letter-spacing:-0.01em;line-height:1.35;color:var(--soft);
}
.tbl-item-time{
...
@media (prefers-reduced-motion:no-preference){
  .tbl-overture{display:grid;animation:tbl-ov-clear .5s var(--ease-soft) 5.35s both;}

  /* the single caret: blinks in, holds through the lines, then drops away */
  .tbl-ov-caret{
    animation:
      tbl-caret-appear .3s var(--ease-soft) .2s both,
      tbl-caret-blink 1.05s steps(1,end) .5s 4 both,
      tbl-caret-drop .5s var(--ease-rack) 4.9s forwards;
  }
  /* each line types out (clip-path wipe L→R) then holds, then clears */
  .tbl-ov-1{animation:tbl-type 1.7s var(--ease-draw) .55s both;}
  .tbl-ov-2{animation:tbl-type 1.7s var(--ease-draw) 2.35s both;}
  /* line 3 types and holds (carried out by the caret drop + layer clear) */
  .tbl-ov-3{animation:tbl-type-hold 1.1s var(--ease-draw) 4.15s both;}

  /* the artifact starts hidden and reveals after the overture hands off */
  .tbl-artifact{opacity:0;animation:tbl-fade .5s var(--ease-soft) 5.45s both;}
  .tbl-kicker{opacity:0;animation:tbl-rise .6s var(--ease-soft) 5.5s both;}
  .tbl-headline{opacity:0;animation:tbl-rise .7s var(--ease-rack) 5.6s both;}
  /* the capture field materialises — the blank line becoming the notebook */
  .tbl-capture{opacity:0;transform:scale(.98);
    animation:tbl-materialise .6s var(--ease-rack) 5.75s both;}
  .tbl-item{
    opacity:0;
    animation:tbl-catch-in .55s var(--ease-rack) both;
    animation-delay:calc(6.05s + var(--i) * .28s);
  }
  .tbl-foot{opacity:0;animation:tbl-rise .6s var(--ease-soft) 6.8s both;}
