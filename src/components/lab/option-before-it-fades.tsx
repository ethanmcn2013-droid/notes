 ...
 * remembered and lost. A column of half-formed thoughts drifts up and
 * fades — the ones you didn't catch — while the bottom line sits solid
 * in ink with a held indigo caret: the one you wrote down.
 *
 * SSR-safe: the settled composition (fade gradient + solid caught line +
 * caret) is the default CSS. The upward drift and caret draw run once on
 * mount, motion-safe only. No JS.
 */
...
.nt2-kicker {
  ...
  letter-spacing: 0.16em; text-transform: uppercase; color: var(--nt2-faint);
}
...
/* ─────────────────────────────────────────────────────────────
   INTRO — motion-safe only. The ghosts drift up and lose a little
   more opacity; the caught line holds and its caret draws + blinks.
   ───────────────────────────────────────────────────────────── */
@media (prefers-reduced-motion: no-preference) {
  .nt2-kicker { animation: nt2-fade 0.7s var(--ease-out, cubic-bezier(0.23,1,0.32,1)) both; }
  .nt2-ghost {
    animation: nt2-drift 1.4s var(--ease-out, cubic-bezier(0.23,1,0.32,1)) both;
    animation-delay: calc((var(--n) - var(--i)) * 0.12s);
  }
  .nt2-caught {
    opacity: 0;
    animation: nt2-catch 0.7s var(--ease-out, cubic-bezier(0.23,1,0.32,1)) 0.65s forwards;
  }
  .nt2-caught-caret {
